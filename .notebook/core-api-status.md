...
- Core read side now exposes tenant-scoped club APIs without the former public `/core` route prefix:
  - `GET /club`
  - `GET /dashboard`
  - `GET /tables`
  - `GET /tables/:tableId`
...
- HTTP command controllers exist for athletes, tables, and competitions. HTTP read controllers exist for club, athletes, tables, ratings, competitions, and dashboard. Club administrative ownership remains with `identity.tenant`; Core exposes only `GET /club` for tenant-scoped current club display. Invitation and scoreboard are domain-only.
