...
describe("IdentityAuthorizationGuard", () => {
  it("libera rota publica", () => {
    const guard = new IdentityAuthorizationGuard(reflector({ [IS_PUBLIC_KEY]: true }), {} as CurrentContextService);

    expect(guard.canActivate(context())).toBe(true);
  });

  it("rota sem metadata exige system_admin", () => {
    const guard = new IdentityAuthorizationGuard(reflector({}), {
      getPrincipalOrThrow: () => ({
        userId: "user-1",
        tenantId: "tenant-1",
        sessionId: "session-1",
  ...
  it("usa system roles explicitas quando declaradas", () => {
    const guard = new IdentityAuthorizationGuard(reflector({ [SYSTEM_ROLES_KEY]: ["system_admin"] }), {
      getPrincipalOrThrow: () => ({
        userId: "user-1",
        tenantId: "tenant-1",
        sessionId: "session-1",
  ...
  it("exige roles de tenant quando declaradas", () => {
    const guard = new IdentityAuthorizationGuard(reflector({ [TENANT_ROLES_KEY]: ["owner"] }), {
  ...
  it("nega tenant role incorreta", () => {
    const guard = new IdentityAuthorizationGuard(reflector({ [TENANT_ROLES_KEY]: ["admin"] }), {
      getTenantOrThrow: () => ({ id: "tenant-1", slug: "acme" }),
      getPrincipalOrThrow: () => ({
        userId: "user-1",
        tenantId: "tenant-1",
